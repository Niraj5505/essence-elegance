...
import { Mail, Lock, User, ArrowRight, LogIn, AlertCircle, ShieldCheck } from 'lucide-react';
...
const RegisterPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.password !== formData.confirmPassword) {
            return setError('The secret keys do not match.');
        }
        ...
        try {
            ...
            if (result.success) {
                // Traditionally we might login directly, but for now we follow the API result
                navigate('/login');
            } else {
                setError(result.message || 'The Inscription could not be completed.');
            }
        } catch (err) {
            setError('The Maison is currently at capacity. Please try later.');
        } finally {
    ...
    return (
        <div className="min-h-screen bg-zinc-50 flex items-center justify-center px-6 py-24 relative overflow-hidden">
            {/* Background Decorative Elements */}
            <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
                <div className="absolute top-[-10%] left-[-5%] w-[40%] h-[40%] bg-gold/5 blur-[120px] rounded-full" />
                ...
                <div className="bg-white/80 backdrop-blur-xl border border-zinc-200/50 p-10 md:p-14 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)]">
                    ...
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-8">
                        <div className="space-y-2">
                            <label className="text-[9px] tracking-[0.3em] uppercase font-black text-zinc-400 ml-1">Full Identity</label>
                            <div className="relative group">
                                <User className="absolute left-0 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-300 group-focus-within:text-gold transition-colors" />
                                <input
                                    type="text"
                                    name="username"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-2">
                                <label className="text-[9px] tracking-[0.3em] uppercase font-black text-zinc-400 ml-1">Secret Key</label>
                                <div className="relative group">
                            ...
                            <div className="space-y-2">
                                <label className="text-[9px] tracking-[0.3em] uppercase font-black text-zinc-400 ml-1">Confirm Key</label>
                                <div className="relative group">
                                    <ShieldCheck className="absolute left-0 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-300 group-focus-within:text-gold transition-colors" />
                                    <input
                                        type="password"
                        ...
                        <div className="pt-8">
                            ...
                            >
                                <span className="relative z-10 text-[10px] tracking-[0.4em] font-black uppercase">
                                    {isSubmitting ? 'Inscribing Membership...' : 'Complete Inscription'}
                                </span>
                    ...
                    <div className="mt-12 pt-8 border-t border-zinc-100 text-center">
                        <p className="text-zinc-400 text-[10px] tracking-widest uppercase font-bold mb-4">Already a Witness?</p>
                        <Link
                            to="/login"
                            className="inline-flex items-center gap-2 text-gold hover:text-zinc-900 transition-all font-serif italic text-lg"
                        >
                            Return to Entry
                            <LogIn className="w-4 h-4 translate-y-0.5" />
                        </Link>
                    </div>
                </div>

                <div className="mt-12 text-center opacity-60">
                    <p className="text-[9px] tracking-[0.2em] uppercase text-zinc-400 px-8">
                        By inscribing, you agree to the Maison's high standards of privacy and our terms of discovery.
                    </p>
                </div>
            </motion.div>
        </div>
